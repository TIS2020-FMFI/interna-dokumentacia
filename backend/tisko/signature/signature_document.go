...
func AddHandleInitVars() {
	...
	con.AddHeaderPost(path.SkillMatrix, getSkillMatrix)
	...
	con.AddHeaderPost(path.Cancels, cancel)
	con.AddHeaderPost(path.Resigns, resign)
	con.AddHeaderGetID(path.TrainingConfirm, confirm)
	con.AddHeaderPost(path.TrainingSaveConfirm, createConfirm)
}